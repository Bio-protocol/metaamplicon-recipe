...
path <- "./cache/cutadapt" # CHANGE to the directory containing your cutadapted fastq files when using your own data
filtpath <- file.path("./cache/filtered") # Filtered files go into the filtered/ subdirectory
...
saveRDS(seqtab.nochim,"./cache/seqtab.nochim.RDS")
dim(seqtab.nochim)
...
# IMPORT METADATA ####
meta <- read_csv("./input/chagos_metadata_clean.csv")
...
taxa <- assignTaxonomy(seqtab.nochim, "./input/taxonomy/rdp_train_set_16.fa.gz", multithread=4) # on Windows, set multithread = FALSE
...
taxa <- addSpecies(taxa, "./input/taxonomy/rdp_species_assignment_16.fa.gz")
